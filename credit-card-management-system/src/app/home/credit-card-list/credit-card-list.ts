...
export class CreditCardList {
  ...
  selectedTransaction = computed(() => this.transactions()
    .filter(t => t.cardNumber === this.selectedCard()?.cardNumber))
  ...
  handleDeleteCard(card: CreditCard) {
    this.creditCardService.deleteCreditCard(card.cardNumber)
      .subscribe({
        next: (response) => {
